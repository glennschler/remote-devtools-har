...
class HarBuilder {
  ...
  addRequest (reqObj, cookies) {
    ...
    if (setRequestArr === undefined) {
      const entry = {
        ...
        startedDateTime: new Date(reqObj.wallTime * 1000).toISOString()
      }
  ...
  */
  addResponse (respObj, respBodyObj, cookies) {
    ...
    if (respObj.hasOwnProperty('errorText')) {
      ...
      requestArr.forEach(internals.addResponse.bind(this,
                            entryObj, respObj, respBodyObj, cookies))
    }
  ...
  get harLog () {
    ...
    return `${JSON.stringify(rcHarLog)}\n`
  }
  ...
  */
  async finishRequest (loadingObj) {
    ...
    const rcStr = `${JSON.stringify(harEntry)}\n`
    return rcStr
  }
}
 ...
 * @method addResponse
*/
internals.addResponse = function (entryObj, respObj, respBodyObj, cookies, reqObj) {
  const regExVer = /HTTP\/\d+\.\d+/

  if (entryObj === reqObj) return // 1st item in the array is not a reqObj
  ...
  internals.addHarEntry.call(this, respObj.requestId, entryObj, reqObj, response)
}

internals.addFailedResponse = function (entryObj, failedObj) {
  const response = {
    status: -1,
  ...
  const { requestId } = failedObj
  internals.addHarEntry.call(this, requestId, entryObj, failedObj, response)
}
 ...
 * @method addHarEntry
*/
internals.addHarEntry = function (requestId, entryObj, reqObj, response) {
  const timings = {
    send: 0,
    wait: 0,
    receive: 0
  ...
  const harRequest = {
    startedDateTime: entryObj.startedDateTime,
    time: 0,
    request: reqObj,
    response: response,
    cache: {},
