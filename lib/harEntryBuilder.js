  ...
  */
  addRequest (reqObj, cookies) {
    ...
    if (this.listenerCount(`event${reqObj.requestId}`) > 0) {
      console.log(`emitting event${reqObj.requestId}`)
  ...
  addResponse (respObj, respBodyObj, cookies) {
    if (this._requestsMap.has(respObj.requestId) === false) {
      ...
      this.once(`event${respObj.requestId}`,
        function(iRespObj, iRespBodyObj, iCookies) {
          console.log(`event${respObj.requestId}`)
          this.addResponse(iRespObj, iRespBodyObj, iCookies)
      }.bind(this, respObj, respBodyObj, cookies))
      return
    }
