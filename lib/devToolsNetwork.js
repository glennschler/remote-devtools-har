  ...
  */
  async enable () {
    try {
      ...
      // Setup the EventEmitter listener to receive every loadingFailed request
      ...
      // Setup the EventEmitter listener to receive every Request
...
internals.handleResponse = async function (respObj) {
  try {
    ...
    const respBodyObj = await this._network.getResponseBody({
    ...
    const count = this._harBuilder.addResponse(respObj, respBodyObj,
                                        cookiesObj.cookies)
    if (count > 0) {
      this._readStream.append(respBodyObj.body)
      console.log(`Item # ${count}, content len = ${respBodyObj.body.length}, ` +
